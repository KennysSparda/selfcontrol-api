...
router.get('/:id', async (req, res) => {
    ...
    try {
        const produtosPorEstoque = await ProdutoEstoqueModel.obterProdutosPorEstoque(estoqueId);
        res.json(produtosPorEstoque);
    } catch (err) {
        console.error(`Erro ao buscar produtos do estoque com ID ${estoqueId}`, err.message);
        res.status(500).send(`Erro ao buscar produtos do estoque com ID ${estoqueId}`);
    }
});
...
router.post('/', async (req, res) => {
    ...
    if (ProdutoID && EstoqueID && Quantidade) {
        try {
            const novaEntrada = await ProdutoEstoqueModel.vincularProdutoAoEstoque(ProdutoID, EstoqueID, Quantidade);
        ...
        console.error(`Alguns dados nao estao chegando: ProdutoID: ${ProdutoID}, EstoqueID: ${EstoqueID}, Qntd: ${Quantidade}`)
    }
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const produtoEstoqueAtualizado = await ProdutoEstoqueModel.atualizarQuantidadeProdutoNoEstoque(ProdutoEstoqueID, ProdutoID, EstoqueID, Quantidade);
