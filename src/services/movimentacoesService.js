...
const criarMovimentacao = async (Data, Quantidade, Tipo, fk_Produto_ID, fk_Funcionario_ID, estoqueid) => {
    try {
        ...
        if (Tipo === 1) { // ID 1 representa movimentação de entrada
            await processarEntrada(Quantidade, fk_Produto_ID, estoqueid);
        } else if (Tipo === 2) { // ID 2 representa movimentação de saída
            const quantidadeDisponivel = await produtoEstoqueService.obterQuantidadeProdutoNoEstoque(fk_Produto_ID, estoqueid);
        ...
        const novaMovimentacao = await movimentacoesModel.criarMovimentacao(Data, Quantidade, Tipo, fk_Produto_ID, fk_Funcionario_ID, estoqueid);
...
const processarEntrada = async (Quantidade,  fk_Produto_ID, estoqueid) => {
    console.log("Processar entrada chamado")
    try {
        const existeProdutoNoEstoque = await produtoEstoqueService.verificarExistenciaProdutoNoEstoque(fk_Produto_ID, estoqueid);

        if (existeProdutoNoEstoque) {
            console.log("movimentacoesService: Existe produto")
            await produtoEstoqueService.atualizarQuantidadeProdutoNoEstoqueService(fk_Produto_ID, estoqueid, Quantidade);
        } else {
            console.log("movimentacoesService: Nao existe produto")
            await produtoEstoqueService.vincularProdutoAoEstoque(fk_Produto_ID, estoqueid, Quantidade);
        }
    } catch (error) {
...
const processarSaida = async (Quantidade,  fk_Produto_ID, estoqueid) => {
    console.log("Processar saida chamado")
    try {
        const existeProdutoNoEstoque = await produtoEstoqueService.verificarExistenciaProdutoNoEstoque(fk_Produto_ID, estoqueid);
